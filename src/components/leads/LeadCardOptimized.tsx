...
import AssignPopoverOptimized from "../common/AssignPopoverOptimized";
import ConfirmModalOptimized from "../common/ConfirmModalOptimized";
...
const DEFAULT_AVATAR = "https://img.freepik.com/premium-vector/person-with-blue-shirt-that-says-name-person_1029948-7040.jpg?w=1380";

const LeadCardOptimized: React.FC<LeadCardProps> = ({ lead }) => {
  ...
  const assignee = useMemo(() => 
    users.find((user) => user.id === lead.assignedTo),
  ...
  const handleAssignConfirm = useCallback(() => {
    if (selectedUserId) {
      assignLead(lead.id, selectedUserId);
      setSelectedUserId(null);
    }
  }, [selectedUserId, lead.id, assignLead]);
  ...
  ), [leadName, lead.location, lead.type, lead.source, lead.description, lead.groupName, lead.time, peopleInvolved, assignee]);

  const leadCardActions = useMemo(() => (
  ...
  ), [lead.score, lead.isUnlocked, lead.creditCost, lead.assignedTo, lead.likeStatus, lead.id, getScoreColor, handleUnlockClick, handleAssignButtonClick, handleViewDetailsClick, likeLead, dislikeLead]);
  ...
  return (
    <>
      ...
      <ConfirmModalOptimized
        isOpen={isUnlockModalOpen}
        onClose={() => setIsUnlockModalOpen(false)}
        onConfirm={handleUnlockConfirm}
        title="Unlock Contact"
        message={`Are you sure you want to unlock this contact for ${lead.creditCost} credits?`}
        confirmText="Unlock"
        type="default"
      />
      ...
      <ConfirmModalOptimized
        ...
        onConfirm={() => setIsViewDetailsModalOpen(false)}
      ...
      <ConfirmModalOptimized
        ...
        title={lead.assignedTo ? "Reassign Contact" : "Assign Contact"}
        message={selectedUser ? `Are you sure you want to ${lead.assignedTo ? 're' : ''}assign this contact to ${selectedUser.name}?` : ''}
        confirmText={lead.assignedTo ? "Reassign" : "Assign"}
        type="default"
      />
    </>
  );
...
export default React.memo(LeadCardOptimized); 