...
const OptimizedLeadCard: React.FC<OptimizedLeadCardProps> = ({ lead }) => {
  const { users, unlockLead, assignLead, likeLead, dislikeLead } = useAppContext();
  const [isAssignPopoverOpen, setIsAssignPopoverOpen] = useState(false);
  const assignButtonRef = useRef<HTMLButtonElement>(null);

  const assignee = useMemo(() => users.find((user) => user.id === lead.assignedTo), [users, lead.assignedTo]);

  const handleUnlockClick = useCallback(() => {
    if (window.confirm(`Unlock this lead for ${lead.creditCost} credits?`)) {
      unlockLead(lead.id);
    }
  }, [lead.creditCost, lead.id, unlockLead]);

  const handleAssign = useCallback((userId: string) => {
    assignLead(lead.id, userId);
  }, [lead.id, assignLead]);

  const handleAssignButtonClick = useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
    event.stopPropagation();
    setIsAssignPopoverOpen((prev) => !prev);
  }, []);

  const getScoreColor = useCallback((score: number | null | undefined): string => {
  ...
  const leadName = useMemo(() => 
    lead.type === "ReceptoNet" && !lead.isUnlocked ? "[Hidden Contact]" : lead.name,
    [lead.type, lead.isUnlocked, lead.name]
  );
  ...
  const cardClassName = useMemo(() => 
    `lead-card card ${lead.type === "ReceptoNet" ? "receptonet-lead" : "orgnetwork-lead"}`,
    [lead.type]
  );

  const scoreColor = useMemo(() => getScoreColor(lead.score), [getScoreColor, lead.score]);

  const renderInvolvedAvatars = useMemo(() => (
  ...
  const renderOrgSpecificFields = useMemo(() => (
  ...
  ), [lead.type, lead.groupName]);

  const renderAssignedInfo = useMemo(() => (
    assignee && (
      <span className="assigned-info" title={`${assignee.name} - Assigned view`}>
        Assigned to:
  ...
  const renderScore = useMemo(() => (
    lead.score !== null && lead.score !== undefined && (
      <div className="lead-score" style={{ backgroundColor: scoreColor }}>
        {lead.score}
      </div>
    )
  ), [lead.score, scoreColor]);

  const renderUnlockButton = useMemo(() => (
    !lead.isUnlocked && (
      <button className="btn btn-secondary unlock-btn" onClick={handleUnlockClick}>
        Unlock Contact ({lead.creditCost} Cr)
      </button>
    )
  ), [lead.isUnlocked, lead.creditCost, handleUnlockClick]);

  const renderUnlockedActions = useMemo(() => (
    lead.isUnlocked && (
  ...
  ), [lead.isUnlocked, lead.assignedTo, handleAssignButtonClick]);

  const renderActionButtons = useMemo(() => (
    <div className="action-buttons">
      <button
        className={`action-btn like-btn ${lead.likeStatus === "liked" ? "active" : ""}`}
      ...
      <button
        className={`action-btn dislike-btn ${lead.likeStatus === "disliked" ? "active" : ""}`}
  ...
  ), [lead.likeStatus, lead.id, likeLead, dislikeLead]);
      ...
      <div className={cardClassName}>
        <div className="lead-card-main">
          <div className="lead-card-avatar">
            <img
              src={lead.type === "OrgNetwork" && lead.people ? "/user-avatar.jpg" : "/company-icon.png"}
              alt={leadName}
            />
          </div>
          <div className="lead-card-content">
            <div className="lead-card-header">
              <div className="lead-card-info">
                ...
                <span className="lead-card-location text-sm text-gray">{lead.location}</span>
              </div>
              {lead.type === "OrgNetwork" && lead.source && (
                <div className="lead-card-source text-sm text-gray">
                  {renderInvolvedAvatars}
                  <span className="source-text">{lead.source}</span>
                </div>
              )}
            </div>
            <p className="lead-card-description text-md mt-2">{lead.description}</p>
            {renderOrgSpecificFields}
...
export default React.memo(OptimizedLeadCard); 