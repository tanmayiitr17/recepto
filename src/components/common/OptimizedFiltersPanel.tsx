...
interface OptimizedFiltersPanelProps {
  selectedFilters: Filters;
  onApply: (filters: Filters) => void;
  onCancel: () => void;
}

const OptimizedFiltersPanel: React.FC<OptimizedFiltersPanelProps> = ({
  selectedFilters,
  onApply,
  onCancel,
}) => {
  const [activeFilterTab, setActiveFilterTab] = useState<keyof Filters>("location");
  const [currentFilters, setCurrentFilters] = useState<Filters>(selectedFilters);
  ...
  const handleLocationCheckboxChange = useCallback((value: string) => {
  ...
  const handleScoreChange = useCallback((value: string | null) => {
    setCurrentFilters((prev) => ({ ...prev, scoreRange: value }));
  }, []);
  ...
  const getAppliedCount = useCallback((filterType: keyof Filters): number => {
  ...
  const filteredLocations = useMemo(() => 
    filterOptions.location.filter((option) =>
    ...
    <div className="filter-section">
      ...
      <div className="filter-options-grid">
        {filteredLocations.map((option) => (
          <div key={option.id} className="filter-option checkbox-option">
            <input
              type="checkbox"
              id={`loc-${option.id}`}
              checked={currentFilters.location?.includes(option.name) || false}
        ...
        {filteredLocations.length === 0 && (
          <p className="text-sm text-gray">No locations match your search.</p>
        )}
      </div>
    </div>
  ), [searchQuery, currentFilters.location, getAppliedCount, handleLocationCheckboxChange, filteredLocations]);

  const renderScoreTab = useMemo(() => (
    <div className="filter-section">
      <h5>
        Score{" "}
  ...
  ), [currentFilters.scoreRange, getAppliedCount, handleScoreChange]);

  const renderFilterTabs = useMemo(() => (
    <div className="filters-sidebar">
      <button
        className={`filter-tab ${activeFilterTab === "location" ? "active" : ""}`}
        onClick={() => setActiveFilterTab("location")}
      >
        Location{" "}
        {getAppliedCount("location") > 0 && (
          <span className="applied-count">{getAppliedCount("location")}</span>
        )}
      </button>
      <button
        className={`filter-tab ${activeFilterTab === "scoreRange" ? "active" : ""}`}
        onClick={() => setActiveFilterTab("scoreRange")}
      >
        Score{" "}
        {getAppliedCount("scoreRange") > 0 && (
          <span className="applied-count">{getAppliedCount("scoreRange")}</span>
        )}
      </button>
    </div>
  ), [activeFilterTab, getAppliedCount]);

  const renderFilterContent = useMemo(() => (
    <div className="filters-content">
      {activeFilterTab === "location" ? renderLocationTab : renderScoreTab}
    </div>
  ), [activeFilterTab, renderLocationTab, renderScoreTab]);
  ...
  return (
    <div className="filters-panel card">
      <div className="filters-header">
        <h4>Filters</h4>
        {totalAppliedCount > 0 && (
          <span className="filters-applied-total">{totalAppliedCount} applied</span>
        )}
      </div>
...
export default React.memo(OptimizedFiltersPanel); 